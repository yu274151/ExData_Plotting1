...
# For reference on dplyr, following this URL: https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html
...
# Calling trunc_data() returns data set on per minute observation from 2007-02-01 to 2007-02-02 (YYYY-MM-DD)

values_read <- trunc_data()

# Because plot2, plot3 and plot4 basically uses a formatted data set for rest of the plots, we will read the data set once into memory from this   point onward


# Modifying values_read through dplyr functions for rest of the plots


# First: used paste function to combine character values from Date and Time column
# Second: used dmy_hms function from lubridate to convert Date and Time column values to datetime
# Note: the representation in data set is DD/MM/YYYY (space) HH:MM:SS; hence, used dmy_hms to parse that character value
# Third: used transmute function from dplyr and saved it in a new field called combined; the returned data set is referred by date_time (named aptly)
# Fourth, other_obsvs refers to data set returned by using select function of dplyr
...
# Note: transmute changes the dataset as desired here and returns one with new variables
...
# Finally we retained interim_values as a function called scope_out_values
# This function is scoped via source call in plot3 and plot4
# It makes more sense to scope out a modified data set rather than calling over and again
